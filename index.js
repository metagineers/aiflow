...
const options = {
  definition: {
    ...
    info: {
      title: "AIFlow",
      version: "1.0.0",
      description: "A Task Oriented Autonomous AI Agents API Framework Toolkit showcasing using tools related to ChainLink, Flow and Crypto related tools",
    },
    servers: [
      {
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (result.stream) {
          ...
          // sometimes the output is in result.output, sometimes in result.text
          if (result.output) {
            res.json({ text: result.output })
          } else {
...
app.listen(port, () => {
  console.log(`AIFlow app listening at http://localhost:${port}`)
})
